...
class AssistantConfig(LocalConf):
    ...
    def _migrate_webcache(self):
        ...
        try:
            ...
            # malformed/truncated file, eg. an interrupted write. leave it
            # in place untouched so a later repair can still migrate it
            LOG.warning(f"failed to parse legacy config cache {WEB_CONFIG_CACHE}, "
                        f"skipping migration")
            return
        if cache:
            # reload from disk first so a concurrent first-boot writer that
            # persisted keys between this object's __init__ load and here
            # is not clobbered by this store()
            self.reload()
